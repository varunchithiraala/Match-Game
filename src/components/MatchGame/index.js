...
class MatchGame extends Component {
  state = {
    ...
    gameImageId: imagesList[0].id,
    gameImageUrl: imagesList[0].imageUrl,
    isGameOver: false,
  }
  ...
  onCheckingImage = imageId => {
    const {gameImageId, timerInSec} = this.state
    ...
    if (gameImageId === imageId) {
      const newGameImage =
        imagesList[Math.ceil(Math.random() * (imagesList.length - 1))]
      this.setState(prevState => ({
        score: prevState.score + 1,
        gameImageId: newGameImage.id,
        gameImageUrl: newGameImage.imageUrl,
      }))
    } else {
      this.clearTimerInterval()
      this.setState({isGameOver: true})
  ...
  render() {
    ...
    const thumbnailsList = imagesList.filter(
      eachThumbnail => eachThumbnail.category === activeTabId,
    )

    return (
      <>
        <NavBar score={score} remainingTime={timerInSec} />
        <div className="match-game-app-container">
          {isGameOver ? (
              ...
              <ul className="tabs-list-container">
                {tabsList.map(eachTab => (
                  <TabItem
                    ...
                    changeTab={this.onChangeTabItem}
                  />
                ))}
              </ul>
              <ul className="thumbnails-list-container">
                {thumbnailsList.map(eachThumbnail => (
                  <ThumbnailItem
                    ...
                    checkingImage={this.onCheckingImage}
                  />
                ))}
              </ul>
            </div>
